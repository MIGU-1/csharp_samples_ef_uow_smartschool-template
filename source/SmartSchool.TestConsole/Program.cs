        ...
        static void Main()
        {
            ...
            using (UnitOfWork unitOfWork = new UnitOfWork())
            {
                ...
                var count = unitOfWork.MeasurementRepository
                    .GetAllMeasurementsByLocationAndName("livingroom", "temperature").Count();

                Console.WriteLine($"Anzahl Messwerte für Sensor temperature in location livingroom: {count}");
                Console.WriteLine();

                var greatestmeasurements = unitOfWork.MeasurementRepository.GetAllMeasurementsByLocationAndName("livingroom", "temperature")
                ...
                var average = unitOfWork.MeasurementRepository.GetValidCo2MeasurementsInOffice()
                    .Average(m => m.Value);
                ...
                var sensorsWithMes = unitOfWork.SensorRepository.GetAllSensors()
                    .OrderBy(s => s.Location)
                    .ThenBy(s => s.Name);

                Console.WriteLine();
                foreach (var sensor in sensorsWithMes)
                {
